new Vue({
  ...
  created() {
    ...
    this.wholeRegisteredUsers = this.getterParsedLocalStorage(
      this.USERS_REGISTERED
    );
    this.currentLoguedUser = this.getterParsedLocalStorage(
      this.CURRENT_USER_LOGUED
    );
  },
  methods: {
    payment(value) {
      this.validations() ? this.error : (this.rechargedValue = value);
    },
    validations() {
